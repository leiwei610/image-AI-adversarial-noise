...
class AnalysisPanel(QWidget):
    ...
    def update_from_path(self, path: str):
        ...
        if not path or not os.path.exists(path):
            self.status_label.setText(f"无图片: {path}")
        ...
        try:
            self.hist_ax.cla()
            self.hist_ax.set_title('灰度直方图')
            self.hist_ax.set_xlabel('强度')
            self.hist_ax.set_ylabel('计数')
            flat = gray.ravel()
            ...
            self.hist_ax.cla()
            self.hist_canvas.draw()
            self.status_label.setText(f"直方图错误: {e}")
            self.status_label.setVisible(True)
        ...
        try:
            ...
            self.fft_ax.cla()
            self.fft_ax.set_title('FFT幅值（对数）')
            ...
            self.fft_ax.cla()
            self.fft_canvas.draw()
            self.status_label.setText(f"FFT错误: {e}")
            self.status_label.setVisible(True)
        ...
        try:
            ...
            self.radial_ax.cla()
            self.radial_ax.set_title('径向频率分布')
            self.radial_ax.set_xlabel('归一化半径')
            self.radial_ax.set_ylabel('平均幅值')
            self.radial_ax.plot(centers, radial)
            self.radial_canvas.draw()
        except Exception as e:
            self.radial_ax.cla()
            self.radial_canvas.draw()
            self.status_label.setText(f"径向分布错误: {e}")
            self.status_label.setVisible(True)
        ...
        try:
            ...
            self.glcm_ax.cla()
            self.glcm_ax.set_title('GLCM特征')
            self.glcm_ax.set_xlabel('偏移')
            self.glcm_ax.set_ylabel('值')
            ...
            self.glcm_ax.cla()
            self.glcm_canvas.draw()
            self.status_label.setText(f"GLCM错误: {e}")
            self.status_label.setVisible(True)
        ...
        try:
            ...
            self.lbp_ax.cla()
            self.lbp_ax.set_title('LBP直方图')
            self.lbp_ax.set_xlabel('模式')
            self.lbp_ax.set_ylabel('频率')
            ...
            self.lbp_ax.cla()
            self.lbp_canvas.draw()
            self.status_label.setText(f"LBP错误: {e}")
            self.status_label.setVisible(True)
        ...
        for ax, canvas in (
            ...
            try:
                ...
                if ax is self.hist_ax:
                    ax.text(0.5, 0.5, '无图片', horizontalalignment='center', verticalalignment='center', transform=ax.transAxes)
                canvas.draw()
            except Exception:
                pass